...
exports.uploadAvatar = function (file, options, callback) {
  var filename = options.filename;

  var newFilename = utility.md5(filename + String((new Date()).getTime())) +
    path.extname(filename);

  var avatar_path = config.avatar.path;
  var base_url    = config.avatar.url;
  var filePath    = path.join(avatar_path, newFilename);
  var fileUrl     = base_url + newFilename;
