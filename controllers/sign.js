...
exports.resetPassword = function(req, res, next){
  ...
  User.getUserByMail(req.body.email, function (err, user) {
    ...
    tools.bcompare(req.body.oldPassword, user.pass, function (err, success) {
      if(!success)
      {
        res.status(404);
        ...
        tools.bhash(req.body.newPassword, function(err, newPasshash){
          ...
          user.save(function(err){
            ...
            return res.send({success:true, msg:'您的密码已经重置。'})
          })
        })
      }
...
exports.resetPasswordToInitial = function (req, res) {
  User.getUserByMail(req.body.email, function(err, user){
    if(user===null){
      res.status(404);
      res.send({success:false, msg:'用户名不存在'});
